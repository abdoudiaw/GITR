...
import numpy as np
from periodictable import oxygen, hydrogen, tungsten, deuterium, helium
...
#        names ='W'
    ...
    A1=material1.mass
    A2=material2.mass
    vth1 = np.sqrt(1.602e-19*te/(1.67e-27*Z1))
    vth2 = np.sqrt(1.602e-19*ti/(1.67e-27*Z2))
    nParticles = int(2000)
    data_dict = {}
...
#        if i < int(nParticles/2):
        if i < int(nParticles/2):
            ...
            data_dict['vx'][i] = np.random.uniform(-vth1,vth1)
            data_dict['vy'][i] = np.random.uniform(-vth1,vth1)
            data_dict['vz'][i] = np.random.uniform(-vth1,vth1)
        else:
            ...
            print("material1.symbol ", material2.symbol )
            data_dict['impurity_id'][i] = i
            data_dict['x'][i] = np.random.uniform(xmin,xmax)
            data_dict['y'][i] = np.random.uniform(ymin,ymax)
            data_dict['z'][i] = np.random.uniform(zmin,zmax)
