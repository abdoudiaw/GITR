...
ParticleData readParticleData(const std::string &ncParticleSourceFile) {
    ...
    data.xpfile.resize(nPfile); data.ypfile.resize(nPfile);
    data.zpfile.resize(nPfile); data.vxpfile.resize(nPfile); data.vypfile.resize(nPfile);
    data.vzpfile.resize(nPfile); data.charge.resize(nPfile); data.amu.resize(nPfile);
    ...
    netCDF::NcVar ncp_amu(ncp.getVar("amu"));
    netCDF::NcVar ncp_Z(ncp.getVar("Z"));
    ...
    data.amu = getVarData<gitr_precision>(ncp_amu);
    data.Z = getVarData<gitr_precision>(ncp_Z);
    ncp.close();
    return data;
    ...
    int nP = particleData.xpfile.size();
    long nParticles = nP;
    ...
    auto& amu = particleData.amu;
    auto& Z = particleData.Z;
    ...
    for (size_t i = 0; i < nParticles; ++i) {
        ...
        gitr_precision particleamu = amu[i];
        gitr_precision ionization = Z[i];

        particleArray->setParticleV(i, x, y, z, vx, vy, vz, ionization, particleamu, particleCharge, dt );
